...
int solve(int n, int m, int kk, int x, int y, vector<road>& roads) {
	...
	for (int i = 0; i < m; ++i) {
		...
		if (roads[i].river) {
			// If road is a river, then add the suitable edges to each copy of the graph
			for (int j = 0; j < k; ++j) {
	    		add(g, wm, len, from, j    , to, j + 1);
	    		add(g, wm, len, from, j + 1, to, j);
	    	}
			// For the last level don't add any further down!!
	    	add(g, wm, len, from, k, to, k);
	  	}
		else {
			// Non river edges. Just add to every level
	    	for (int j = 0; j <= k; ++j) {
	...
	// Source - Destination
	// From source on first level to destination on final level	
	int from = nr(x, 0);
	int to = nr(y, k);
