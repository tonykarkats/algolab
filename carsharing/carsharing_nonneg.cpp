...
void testcase() {
	...
	for (int station = 0; station < nr_stations; station++) {
		for (int i=0; i<nr_nodes_per_branch-1; i++) {
			...
			addEdge(from_node, to_node, INT_MAX, 100, capacity, weight, rev_edge, G);
	...
	//	cout << "Inputing request " << i << endl;
		...
		int duration = r.arrival_time / 30 - r.departure_time / 30;
	...
	*/
	...
	long cost = find_flow_cost(G) - (nr_nodes_per_branch -1) * 100 * flow;
	cout << -cost << endl;
